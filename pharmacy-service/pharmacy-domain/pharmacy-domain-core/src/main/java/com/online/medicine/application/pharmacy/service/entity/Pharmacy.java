...
public class Pharmacy extends AggregateRoot<PharmacyId> {
    ...
    public void validateOrder(List<String> failureMessages) {
        ...
        Money totalAmount = orderDetail.getMedicines().stream().map(medicine -> {
            if (!medicine.isAvailable()) {
                failureMessages.add("Medicine " + medicine.getId()
                        + " is not available");
            }
            return medicine.getPrice().multiply(medicine.getQuantity());
        }).reduce(Money.ZERO, Money::add);
        if (!totalAmount.equals(orderDetail.getTotalAmount())) {
            failureMessages.add("Total amount of medicines do not match with total amount in order" + orderDetail.getId());
    ...
    public void constructOrderApproval(OrderApprovalStatus orderApprovalStatus) {
        this.orderApproval = OrderApproval.Builder.newBuilder()
